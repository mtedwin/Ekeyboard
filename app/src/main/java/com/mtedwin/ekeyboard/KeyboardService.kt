...
class KeyboardService : InputMethodService(), KeyboardView.OnKeyboardActionListener {
    ...
    private lateinit var prediction10: TextView
...
//        unregisterReceiver(reloadReceiver)
    ...
    override fun onCreateInputView(): View {
        ...
        prediction7 = rootView.findViewById(R.id.prediction7)
        prediction8 = rootView.findViewById(R.id.prediction8)
        prediction9 = rootView.findViewById(R.id.prediction9)
        prediction10 = rootView.findViewById(R.id.prediction10)
        ...
        prediction7.setOnClickListener { selectPrediction(prediction7.text.toString()) }
        prediction8.setOnClickListener { selectPrediction(prediction8.text.toString()) }
        prediction9.setOnClickListener { selectPrediction(prediction9.text.toString()) }
        prediction10.setOnClickListener { selectPrediction(prediction10.text.toString()) }
    ...
    private suspend fun updatePredictionBar() {
        if (isChinese && isQuickChi) {
            ...
            prediction7.visibility = View.VISIBLE
            prediction8.visibility = View.VISIBLE
            prediction9.visibility = View.VISIBLE
            prediction10.visibility = View.VISIBLE

            val input = currentKeyGroups.joinToString("")
            if (input.length == 2) {
                val firstChar = input[0]
                val lastChar = input[1]

                val predictions = chineseDictionary
                    .filter { (code, _) ->
                        code.length >= 2 &&
                                code.first() == firstChar &&
                                code.last() == lastChar
                    }
                ...
                prediction7.text = predictions.getOrNull(6) ?: ""
                prediction8.text = predictions.getOrNull(7) ?: ""
                prediction9.text = predictions.getOrNull(8) ?: ""
                prediction10.text = predictions.getOrNull(9) ?: ""
            } else {
                prediction1.text = ""
                prediction2.text = ""
                prediction3.text = ""
        ...
        val predictions = withContext(Dispatchers.Default) {
            ...
            // Generate combinations on-demand and check dictionary
            val exactMatches = mutableListOf<String>()
            val shorterMatches = mutableListOf<String>()
            val prefixMatches = mutableListOf<String>()

            // Use prefix map for efficiency
            for (combo in getCombinations(groups, 5000)) {
                val exact = dictionary.filter { it == combo && it.length == exactLength }
                exactMatches.addAll(exact)

                if (exactMatches.size >= 3) break
            }

            // If not enough exact matches, look for prefix matches
            if (exactMatches.size < 3) {
                for (combo in getCombinations(groups, 5000)) {
                    val matches = prefixMap[combo] ?: continue
                    for (word in matches) {
                        if (word.length >= exactLength && word !in exactMatches && word !in prefixMatches) {
                            prefixMatches.add(word)
                        }
                        if (exactMatches.size + prefixMatches.size >= 3) break
                    }
                    if (exactMatches.size + prefixMatches.size >= 3) break
                }
            }

            (exactMatches + prefixMatches).take(3)
        }
