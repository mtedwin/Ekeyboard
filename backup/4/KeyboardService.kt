...
class KeyboardService : InputMethodService(), KeyboardView.OnKeyboardActionListener {
    ...
    fun onT13KeyPress(keyLabel: String) {
        ...
        // Check if we've reached max input length for T13C mode
        if (isT13C && currentKeyGroups.size >= MAX_INPUT_LENGTH) {
            // Auto-commit the top prediction
            predictionJob?.cancel()
            predictionJob = serviceScope.launch {
                updatePredictionBar()
                delay(100) // Small delay to ensure predictions are updated
        ...
        when (primaryCode) {
            ...
            -105 -> { // Symbol keyboard
                ...
                if (isT13) {
                    ...
                    // Check if we've reached max input length for Chinese mode
                    if (currentKeyGroups.size >= MAX_INPUT_LENGTH) {
                        // Auto-commit the top prediction
                        predictionJob?.cancel()
                        predictionJob = serviceScope.launch {
